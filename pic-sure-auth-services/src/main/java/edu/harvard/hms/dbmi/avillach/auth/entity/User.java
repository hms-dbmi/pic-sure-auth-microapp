...
@Table(uniqueConstraints = @UniqueConstraint(columnNames = {"email", "connectionId"}))
@Entity(name = "user")
public class User extends BaseEntity implements Serializable, Principal {
    ...
    @JoinTable(
        name = "user_role", joinColumns = {@JoinColumn(name = "user_id", nullable = false, referencedColumnName = "uuid")},
    ...
    /**
     * <p>NOTICE</p> <br><p> When you update or create a user, please use connection.id as the input. The UserService is specifically using
     * connection.id. </p> <br> <p><b> Note: This is because of the checkAssociation() method in UserService. </b></p><br>
     */
    @ManyToOne
    ...
    public Set<String> getPrivilegeNameSet() {
        ...
        if (totalPrivilegeSet == null) return null;
    ...
    public Set<String> getPrivilegeNameSetByApplication(Application application) {
        ...
        Set<String> nameSet = new HashSet<>();
        if (application == null) return nameSet;

        for (Privilege appPrivilege : application.getPrivileges()) {
            for (Privilege userPrivilege : totalPrivilegeSet) {
                if (appPrivilege.equals(userPrivilege)) nameSet.add(userPrivilege.getName());
            }
        }
        return nameSet;
    }
    ...
    public Set<Privilege> getPrivilegesByApplication(Application application) {
        ...
        roles.forEach(
            r -> privileges.addAll(
                r.getPrivileges().stream()
                    .filter(p -> application.getUuid().equals((p.getApplication() == null) ? null : p.getApplication().getUuid()))
    ...
    public String getRoleString() {
        return (roles == null) ? null : roles.stream().map(Role::getName).collect(Collectors.joining(","));
    }
    ...
    public String toString() {
        ...
        return uuid.toString() + " ___ " + subject + " ___ " + email + " ___ " + generalMetadata + " ___ " + auth0metadata + " ___ {"
            + ((connection == null) ? null : connection.toString()) + "}";
    }
}
